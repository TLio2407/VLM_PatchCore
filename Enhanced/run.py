        ...
        for defect_type in os.listdir(root):
            ...
            for img_name in os.listdir(defect_dir):
                if img_name.endswith((".png", ".jpg", ".jpeg")):
                    mapped = DEFECT_LABEL_MAP.get(defect_type, defect_type)
                    self.samples.append((os.path.join(defect_dir, img_name), mapped))

    def __len__(self):
        return len(self.samples)
    
    def __get_path__(self, idx):
...
for obj in ALL_OBJECTS:
    ...
    # -----------------------------
    preds, gts = [], []
    threshold = 0.5  # arbitrary; adjust per-object if needed

    for idx in range(len(test_dataset)):
        img, lbl = test_dataset[idx]
        score = model.score(img)

        if lbl == "good" and score <= threshold:
            pred_class = "good"
        else:
            pred_class = model.classify_anomaly(img)

        preds.append(pred_class)
        gts.append(lbl)

    print(f"\n🔍 Classification report for {obj}:")
    print(classification_report(gts, preds, labels=model.anomaly_classes))
    print("Confusion Matrix:")
    print(confusion_matrix(gts, preds, labels=model.anomaly_classes))

    # -----------------------------
    # Save logs
    # -----------------------------
    log_path = os.path.join(log_dir, f"{obj}_results.txt")
    with open(log_path, "w") as f:
        for path, gt, pred in zip([p for p, _ in test_dataset.samples], gts, preds):
            f.write(f"{path}: GT={gt}, Pred={pred}\n")

    print(f"✅ Saved classification results for {obj} to {log_path}")
    ...
    for idx in range(min(10, len(test_dataset))):  # only save first 10
        img, lbl = test_dataset[idx]
